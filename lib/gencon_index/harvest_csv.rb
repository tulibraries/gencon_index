...
module GenconIndex
  module HarvestCSV
    ...
    def harvest(csv_source,
      ...
      solr = solr_client || RSolr.connect(url: solr_endpoint)
    ...
    def make_map(csv_path, map_path, id_field)
      schema_map = {}
    ...
    def get_blacklight_add_fields(schema_map, field_match)
      partial_fields = []
      schema_map.values.flatten.select do |field_name|
        next unless field_name.end_with?(field_match)

        partial_fields << {
          field: field_name.parameterize,
          label: field_name.sub(/_#{field_match}$/, "").titleize
        }
      end
      partial_fields
    end

    def blacklight(map_source = "solr_map.yml", partial_output = "_blacklight_config.rb")
      schema_map = YAML.load_file(map_source)
      partial_file = partial_output.respond_to?(:write) ? partial_output : File.new(partial_output, "w")
      line = String.new
      get_blacklight_add_fields(schema_map, "facet").each do |field|
        line << "    config.add_facet_field '#{field[:field]}', label: '#{field[:label]}'\n"
      end
      get_blacklight_add_fields(schema_map, "display").each do |field|
        line << "    config.add_show_field '#{field[:field]}', label: '#{field[:label]}'\n"
      end
